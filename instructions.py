...
class FontGenerator:
    ...
    def get_values_sv(self):
        return [f"64'h{hex(v)[2:]}" for v in self.table]
...
class Formatter:
    ...
    def render_sv(self, output_file):
        sv_file = output_file + ".sv"
        sv_hash_file = sv_file + ".md5"
        sv_file = Path(sv_file)
        sv_hash_file = Path(sv_hash_file)

        self.check_hash(sv_file, sv_hash_file)
...
"""
        ...
        sv_template = Environment(loader=FileSystemLoader(self.template_path)).get_template("sv_pkg.sv.j2")
        sv_data = sv_template.render({
            "warning": warning,
            ...
            "ascii_glyphs": self.data.font.get_values_sv(),
        })

        with sv_file.open("w") as f:
            f.write(sv_data)
        with sv_hash_file.open("w") as f:
            f.write(hex(int.from_bytes(md5(sv_data.encode("utf-8")).digest(), "big"))[2:])
...
def main():
    ...
    parser.add_argument("--sv", nargs="?", const="vpu_defs_pkg", help="Generate SystemVerilog defs")
